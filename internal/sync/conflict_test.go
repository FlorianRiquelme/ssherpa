...
func TestDetectConflicts_ExcludesSshjesusConfig(t *testing.T) {
	...
	tmpDir := t.TempDir()
	sshConfigPath := filepath.Join(tmpDir, "config")
	sshjesusConfigPath := filepath.Join(tmpDir, "sshjesus_config")

	// Create sshjesus include file with 1Password servers
	sshjesusConfig := `# Generated by sshjesus - DO NOT EDIT MANUALLY
# Source: 1Password
...
Host prod-web
    ...
    User deploy
`
	err := os.WriteFile(sshjesusConfigPath, []byte(sshjesusConfig), 0600)
	require.NoError(t, err)

	// Create main SSH config with Include directive
	sshConfig := `Include ` + sshjesusConfigPath + `
...
`
	...
	// Create 1Password server with same name as in sshjesus_config
	onePasswordServers := []*domain.Server{
		{
			ID:          "srv-001",
			DisplayName: "prod-web",
	...
	// Verify no conflicts (sshjesus_config entries should be excluded)
	assert.Len(t, conflicts, 0, "Expected no conflicts when server only exists in sshjesus_config")
}
