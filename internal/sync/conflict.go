...
//
// Entries from the ssherpa_config include file are excluded from conflict detection
// (they're generated by ssherpa itself, so they're not real conflicts).
func DetectConflicts(onePasswordServers []*domain.Server, sshConfigPath string) ([]Conflict, error) {
	...
	// Exclude entries from ssherpa_config (check if they have the ssherpa header comment nearby)
	sshHostMap := make(map[string]*domain.Server)
	for _, host := range sshHosts {
		// Skip if this host is from ssherpa_config
		// We can detect this by checking if SourceFile contains "ssherpa_config"
...
// isSshjesusGenerated checks if an SSH host entry was generated by ssherpa.
// We detect this by checking if the SourceFile path contains "ssherpa_config".
func isSshjesusGenerated(host sshconfig.SSHHost) bool {
	return strings.Contains(host.SourceFile, "ssherpa_config")
}
