...
- `undoStatusStyle` - Accent color for "press 'u' to undo" flash message
  ...
  <verify>`go build ./...` compiles. `go vet ./...` clean. `go test -race ./...` all pass. Run `go run ./cmd/ssherpa/`, press 'd' on a server, verify confirmation prompt appears.</verify>
...
Present the user with the verification steps below. Wait for their "approved" or issue description before proceeding.
  ...
  <verify>
Run `go run ./cmd/ssherpa/` and test the complete CRUD flow:
