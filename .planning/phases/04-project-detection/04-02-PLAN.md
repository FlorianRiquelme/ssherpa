...
Purpose: This transforms the flat server list into a project-organized view that automatically surfaces relevant servers for the user's current git project. This is the core UX differentiator of sshjesus.
...
```
1. Build server-to-projects lookup from m.projects (match by checking if any
   project.GitRemoteURLs contains a URL that produces the same org/repo as a
   server's host alias -- but actually, since sshconfig hosts don't have
   ProjectIDs yet, we need to match via config.Projects[].ID against...

   IMPORTANT: Currently the TUI works with sshconfig.SSHHost structs (from
   SSH config parser), NOT domain.Server structs. SSH config hosts don't have
   ProjectIDs. The project-to-server mapping must come from config.Projects.

   For Phase 4, project assignment is stored in Config.Projects. Each ProjectConfig
   has an ID and Name. Server-to-project assignments are persisted as part of the
   project config (we need to add a ServerNames []string field to ProjectConfig
   that lists SSH config host names belonging to this project -- OR we store the
   mapping separately).

   SIMPLEST APPROACH: Add a field to ProjectConfig:
   ServerNames []string `toml:"server_names"` // SSH host aliases in this project

   Then in rebuildListItems, for each host, check which projects include it.
```

Wait -- let me reconsider. The domain model has `Server.ProjectIDs` for the many-to-many relationship. But the TUI currently works with `sshconfig.SSHHost` directly (not domain.Server). We need a bridge.

**Pragmatic approach for Phase 4:** Since we're still using sshconfig backend (which produces SSHHost structs, not domain.Server), we need to map SSH host names to projects via config. Add `ServerNames []string` to `ProjectConfig` in config.go (this was not explicitly created in Plan 01 -- update config.go here if needed).

Actually, re-reading Plan 01: `ProjectConfig` has `ID`, `Name`, `GitRemoteURLs`, `Color`. We need to also add `ServerNames` to track which SSH hosts belong to each project.

**Add to `ProjectConfig` (in `internal/config/config.go`):**
```go
...
1. **Without projects configured** (baseline check):
   - Run `go run ./cmd/sshjesus/` from any directory
...
2. **With projects configured** (manually edit TOML config):
   - Create config at `~/.config/sshjesus/config.toml` with:
     ```toml
     version = 1
     backend = "sshconfig"
