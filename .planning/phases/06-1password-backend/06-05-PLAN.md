...
must_haves:
  truths:
    ...
    - "Migration wizard scans for existing SSH/Server items in 1Password without ssherpa tag"
  ...
  key_links:
    ...
    - from: "internal/tui/migration.go"
      ...
      via: "tags existing items with ssherpa tag"
      pattern: "onepassword"
...
<objective>
Implement setup wizard and migration wizard: first-launch experience for backend selection, step-by-step 1Password configuration, and migration of existing unstructured SSH items to ssherpa format.

Purpose: Users need a guided onboarding experience to configure the 1Password backend. Teams with existing unstructured SSH items in 1Password need a migration path to bring those items under ssherpa management.
...
that are not yet managed by ssherpa.
...
Verify main.go checks for empty backend: `grep "Backend.*==" cmd/ssherpa/main.go`
  </verify>
  <done>
...
Migration wizard scans all vaults for SSH/Server category items WITHOUT the "ssherpa" tag and offers to tag them.
...
- WITHOUT "ssherpa" tag (already managed items are excluded)
...
1. If complete: add "ssherpa" tag, ensure field labels match expected format (hostname, user, port, etc.)
...
Verify scan logic: `grep "ssherpa" internal/tui/migration.go` shows tag filtering.
  </verify>
  <done>
Migration wizard scans 1Password vaults for unmanaged SSH items. Users select which items to migrate with visibility into completeness. Selected items get tagged with "ssherpa" and field labels normalized. Migration state tracked in config to avoid re-offering.
  </done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 3: End-to-end 1Password backend verification</name>
  <files>cmd/ssherpa/main.go</files>
  <action>
...
1. Delete or rename ssherpa config: rm ~/.config/ssherpa/config.toml
2. Run: go run ./cmd/ssherpa/
...
1. Launch ssherpa (config now saved)
2. Verify servers from ~/.ssh/config appear in list
3. If any 1Password items are tagged "ssherpa", verify they also appear
...
Test 3 - Status Bar:
1. With ssherpa running, lock 1Password (Cmd+L or timeout)
...
1. Check ~/.ssh/ssherpa_config exists with correct Host blocks
