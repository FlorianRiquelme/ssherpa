...
Purpose: This is the core user-facing deliverable of Phase 3. After this plan, users can launch sshjesus, fuzzy-search for a server, press Enter, and be connected via SSH — the primary workflow of the application.
...
@cmd/sshjesus/main.go — Current main (pass config to TUI)
@go.mod — Dependencies
</context>
...
<task type="auto">
  ...
  <files>internal/config/config.go, internal/config/config_test.go, internal/tui/keys.go, internal/tui/model.go, internal/tui/list_view.go, internal/tui/detail_view.go, internal/tui/styles.go, internal/tui/messages.go, cmd/sshjesus/main.go</files>
  <action>
...
Run `go run ./cmd/sshjesus/` — TUI launches with search bar visible at top.
  </verify>
  <done>
...
- History file created at ~/.ssh/sshjesus_history.json after first connection
...
- Connection history tracked in ~/.ssh/sshjesus_history.json
