...
<objective>
Implement the mock backend, config management, and comprehensive tests using TDD for ssherpa.
  ...
  <action>
    ...
    **GREEN phase — implement to pass:**
    ...
    4. Create `internal/config/config.go` with `package config`:
       ...
       - `func DefaultPath() (string, error)` — returns `xdg.ConfigFile("ssherpa/config.toml")` (creates dirs if needed)
       - `func Load(path string) (*Config, error)`:
         - If path is empty, use `xdg.SearchConfigFile("ssherpa/config.toml")`; if search fails, return `ErrConfigNotFound` (from internal/errors)
