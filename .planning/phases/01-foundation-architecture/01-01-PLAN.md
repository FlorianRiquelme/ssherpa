...
must_haves:
  ...
  artifacts:
    ...
    - path: "go.mod"
      ...
      contains: "module github.com/florianriquelme/ssherpa"
  key_links:
...
<objective>
Establish the Go module, domain models, error types, and backend interface contracts for ssherpa.
...
<task type="auto">
  ...
  <action>
    1. Initialize Go module: `go mod init github.com/florianriquelme/ssherpa` (use Go 1.24+).

    2. Create minimal `cmd/ssherpa/main.go` with `package main` and a placeholder `main()` that prints a version string. This is the entry point scaffold — future phases will wire it up.
...
<task type="auto">
  ...
  <action>
    1. Create `internal/errors/errors.go` with `package errors` (shadow stdlib errors intentionally — re-export what's needed):
       ...
       - Re-export `errors.Is`, `errors.As`, `errors.New`, `errors.Unwrap` from stdlib so callers can import just `ssherpa/internal/errors` instead of both packages
